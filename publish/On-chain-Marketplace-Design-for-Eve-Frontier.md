---
title: On-Chain Marketplace Design for EVE Frontier
excerpt: How to build an on-chain marketplace in EVE Frontier - order book DEX versus AMM designs, custody in storage units, settlement, and order discovery.
tags:
  - ecosystem
  - onchain-markets
...
On-chain marketplaces offer a solution by making trade infrastructure **universally accessible**. By replacing NPC trading hubs with smart contract-powered structures, players can establish [distributed, self-sustaining markets](/features/marketplace) anywhere in the galaxy.
...
For players to interact with [trade offers](/marketplace/discovery), they must be able to _find_ them. Discovery is how listings become visible to buyers, and indexing is how they are organized and retrieved efficiently. On-chain discovery is non-trivial - but there are a couple of approaches that can be taken:
- Smart contracts can **emit events** for off-chain indexers to consume. This approach is more “gas efficient” (cheaper to execute on the blockchain due no consumption of state - the most expensive part of a blockchain transaction) at the expense of being more expensive to operate for the marketplace operator.
